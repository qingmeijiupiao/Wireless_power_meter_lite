...
esp_err_t read_register(Register_enum reg, uint16_t* val) {
    ...
    uint8_t   data[2];
    esp_err_t ret = lp_core_i2c_master_write_read_device(LP_I2C_NUM_0, I2C_ADDR, &reg_byte, 1, data, 2, I2C_TIMEOUT);
    if (ret != ESP_OK) {
        return ret;
    }
...
esp_err_t write_register(Register_enum reg, uint16_t val) {
    ...
    esp_err_t ret = lp_core_i2c_master_write_read_device(LP_I2C_NUM_0, I2C_ADDR, data, 3, &reg_byte, 1, I2C_TIMEOUT);
    if (ret != ESP_OK) {
        return ret;
    }
    return ESP_OK;
 ...
 */
esp_err_t set_configuration(Avg_times_enum avg_times, Timing_enum shunt_timing, Timing_enum bus_timing,
                            Mode_enum mode) {
    uint16_t config = (static_cast<uint16_t>(avg_times) << 9) | (static_cast<uint16_t>(shunt_timing) << 6) |
                      (static_cast<uint16_t>(bus_timing) << 3) | static_cast<uint16_t>(mode);
