...
- **原始寄存器调试**：启动成功后设置 `current_register_raw` 和 `voltage_register_raw` 指针，供 Shell 查询
...
- 启动成功后才设置 `current_register_raw` / `voltage_register_raw`，因此 Shell 的 `ina226_register` 命令依赖 LP 核成功启动。
