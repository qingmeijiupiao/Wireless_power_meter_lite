...
class CurveHistory {
  public:
     ...
     */
    size_t build_buckets(CurveMetric metric, uint32_t window_ms, CurveBucket* buckets, size_t bucket_count) const;
  ...
  private:
    struct Sample {
        uint16_t voltage_mV = 0; /**< 电压，单位 1mV/LSB。 */
        uint16_t current_mA = 0; /**< 电流绝对值，单位 1mA/LSB。 */
    };

    static_assert(sizeof(Sample) == 4, "CurveHistory::Sample must stay compact");

    /** @brief `CurveHistory` 接口。 */
