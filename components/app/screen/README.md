...
| 电量页 | `BatteryPage` | 250ms | 无标题三行布局：累计 `mWh`、`mAh`，以及同一行显示的系统时间和计量时间；长按侧键清零页面累计值并重新开始计量时间 |
| 曲线页 | `CurvePage` | 200ms | 当前为曲线功能占位页面 |
| 无线页 | `WirelessPage` | 500ms | 显示 WiFi 模式；STA 显示 SSID、IP、信号强度百分比和信道；长按侧键进入 AP 配网 |
