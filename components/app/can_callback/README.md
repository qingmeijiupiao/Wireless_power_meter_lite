...
- **NVS 参数**：设备 ID 通过 `NVS_DATA` 持久化，初始化注册回调时读取；`CAN_BAUDRATE` 也有 NVS 变量和 Shell 命令，但当前初始化代码仍固定以 `1_Mbps` 创建 `HXC_TWAI`
...
flowchart TD
    ...
    StateCb --> Bus["new HXC_TWAI(CAN_TX, CAN_RX, 1_Mbps)"]
...
| 变量 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `CAN_BAUDRATE` | `NVS_DATA<uint32_t>` | `1_Mbps` | CAN 波特率配置值；当前 `init()` 创建总线时仍固定传入 `1_Mbps` |
