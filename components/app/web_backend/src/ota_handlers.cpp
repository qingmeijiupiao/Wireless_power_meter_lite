...
const char* ota_state_to_str(OtaManager::State state) {
    switch (state) {
    case OtaManager::State::IDLE:
        return "idle";
    case OtaManager::State::WRITING:
        return "writing";
    case OtaManager::State::VERIFIED:
        return "verified";
    case OtaManager::State::READY_TO_REBOOT:
...
const char* ota_error_to_str(esp_err_t err) {
    ...
    case ESP_ERR_OTA_ROLLBACK_INVALID_STATE:
        return "running_firmware_not_confirmed";
    default:
...
esp_err_t ota_upload_handler(WebServer::Request* request) {
    ...
    if (image_size == 0) {
        ...
        return WebServer::send(request, 400, "application/json", "{\"ok\":false,\"reason\":\"empty_image\"}\n",
    ...
    if (err != ESP_OK) {
        ...
        return WebServer::send(request, err == ESP_ERR_INVALID_SIZE ? 413 : 409, "application/json", web_scratch_buffer,
                               strlen(web_scratch_buffer));
    }
    append_ota_status_event("upload_started");

    uint8_t next_progress_percent = 25;
    err                           = WebServer::stream_body(
        request, web_scratch_buffer, sizeof(web_scratch_buffer),
...
esp_err_t ota_remote_check_handler(WebServer::Request* request) {
    ...
    if (err != ESP_OK) {
        snprintf(response_buffer, sizeof(response_buffer), "{\"ok\":false,\"reason\":\"%s\"}\n", ota_error_to_str(err));
        return WebServer::send(request, err == ESP_ERR_INVALID_STATE ? 409 : 500, "application/json", response_buffer,
