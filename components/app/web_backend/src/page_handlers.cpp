...
esp_err_t charts_page_handler(WebServer::Request* request) {
    return WebServer::send_html_gzip(request, charts_html_file.data, charts_html_file.size);
}

/** @brief 返回控制设置页面。 */
esp_err_t control_page_handler(WebServer::Request* request) {
    return WebServer::send_html_gzip(request, control_html_file.data, control_html_file.size);
}

/** @brief 返回状态诊断页面。 */
esp_err_t status_page_handler(WebServer::Request* request) {
    return WebServer::send_html_gzip(request, status_html_file.data, status_html_file.size);
}

/** @brief 返回实时日志页面。 */
esp_err_t logs_page_handler(WebServer::Request* request) {
    return WebServer::send_html_gzip(request, logs_html_file.data, logs_html_file.size);
}

/** @brief 返回黑匣子日志入口页面。 */
esp_err_t blackbox_page_handler(WebServer::Request* request) {
    return WebServer::send_html_gzip(request, blackbox_html_file.data, blackbox_html_file.size);
}

/** @brief 返回固件升级页面。 */
esp_err_t firmware_page_handler(WebServer::Request* request) {
    return WebServer::send_html_gzip(request, firmware_html_file.data, firmware_html_file.size);
}

/** @brief 返回 Web 公共样式表。 */
esp_err_t app_css_handler(WebServer::Request* request) {
    return WebServer::send_gzip(request, 200, "text/css", app_css_file.data, app_css_file.size);
}
